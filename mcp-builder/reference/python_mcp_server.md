...
FastMCP 提供了 get_http_headers() 函数，用于在工具函数中获取当前请求的 headers。
...
> 注意：这个方式依赖于 FastMCP 的 HTTP 模式，并且需要确保请求是通过 HTTP 传输的（如 SSE 或 streamable-http） 。

如果你使用的是 stdio 模式（标准输入输出），则无法访问 HTTP headers，因为根本没有 HTTP 层。
使用 get_http_headers() 时，确保服务端是以 HTTP 模式运行的，例如：
...
| 场景          | 客户端设置 headers                     | 服务端读取 headers                     |
| ------------- | -------------------------------------- | -------------------------------------- |
| HTTP/SSE 模式 | `StreamableHttpTransport(headers=...)` | `get_http_headers()` 或 FastAPI 中间件 |
| stdio 模式    | 不支持                                 | 不支持                                 |
